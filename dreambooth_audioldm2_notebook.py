...
def log_validation(audioldmpipeline, text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, global_step,vocoder,concept_audio_dir, validate_experiments=False):
    ...
    for _ in range(args.num_validation_audio_files):
        ...
        audio_gen = pipeline(args.validation_prompt, num_inference_steps=100,audio_length_in_s=5.0).audios[0]
    ...
    if validate_experiments:
        ...
        #audios_rec = pipeline(args.validation_prompt, num_inference_steps=100,audio_length_in_s=10.24, generator=generator,num_waveforms_per_prompt=10).audios
        audios_rec = pipeline(args.validation_prompt, num_inference_steps=100, audio_length_in_s=5.0,
                              generator=generator, num_waveforms_per_prompt=10).audios
...
def parse_args():
    ...
    if args.instance_word and args.object_class:
        #args.validation_prompt = f"a sound of a {args.instance_word} {args.object_class}"
        #args.class_prompt = f"a sound of a {args.object_class}"
        args.validation_prompt = f"{args.instance_word} {args.object_class}"
        args.class_prompt = f"{args.object_class}"
...
def read_wav_file(filename, segment_length, augment_data=False):
    ...
    if reverb:
        print("-------------------------------------------------------")
        board = Pedalboard([Reverb(room_size=0.80)])
        waveform = board(waveform, sample_rate=16000)
    '''
...
class AudioInversionDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.instance_word and self.class_name:
            ...
            #text = "a sound of a {}".format(self.instance_word+" "+self.class_name )
            text = "{}".format(self.instance_word + " " + self.class_name)
...
def train(jupyter=False,
          MODELNAME=None, DATAPATH="", OUTPATH=None,
          INSTANCEWORD=None, OBJECTCLASS=None,TRAINSTEPS=100,
          LEARNINGRATE=4.0e-06,TRAINBATCHSIZE=1,
          GRADIENTACCUMULATION=4, NUMVECTORS=2,
          #VALIDATIONSTEPS=500,
          ):
    ...
    if jupyter:
        #------------------------------------------------------------------------------------
        args.pretrained_model_name_or_path=MODELNAME
        args.train_data_dir=DATAPATH
        args.output_dir=OUTPATH
        args.instance_word = INSTANCEWORD
        args.object_class=OBJECTCLASS
        args.max_train_steps=TRAINSTEPS
        args.validation_prompt = f"{args.instance_word} {args.object_class}"
        #args.validation_steps = VALIDATIONSTEPS
        args.learning_rate = LEARNINGRATE
        args.train_batch_size = TRAINBATCHSIZE
        args.gradient_accumulation_steps = GRADIENTACCUMULATION
        args.num_vectors = NUMVECTORS
        args.save_as_full_pipeline = True
        #------------------------------------------------------------------------------------
    ...
    print(f"Dataset : {args.train_data_dir}")
    print(f"Output : {args.output_dir}")
    print(f"{args.instance_word},{args.object_class},Train steps : {args.max_train_steps}")
    print(f"Learning rate = {args.learning_rate}")
    print(f"Save Full Model : {args.save_as_full_pipeline}")
    print(f"Validation prompt : {args.validation_prompt}")
    #print(f"Number of vectors : {args.num_vectors}")
    print("----------------------------------------------------------------------------------")
    ...
    audioldmpipeline= AudioLDM2Pipeline.from_pretrained(
        ...
        safety_checker=None,
        revision=args.revision,
        torch_dtype=torch.float32, #change it to weight_dtype
    ).to(accelerator.device)
    ...
    with open(os.path.join(args.output_dir, f"class_name_{args.instance_word}_{args.object_class}.json"), "w") as fd:
        if args.instance_word and args.object_class:
            data = {
