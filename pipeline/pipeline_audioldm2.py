...
def string_matching(s1,s2):
    '''
    Function that checks if the first word (keyword) of the string s2 is present in string s1
    :param s1: reference string [string], in this case is the text prompt
    :param s2: string to check [string], in this case is the name of the file in validation set folder
    :return: True if the keyword is  [Boolean]
    '''
    #Program part for all the element of s2 in s1

    words_s1 = s1.split()
    words_s2 = s2.split('_')
    words_s2.pop()
    # Verifica la presenza di ogni parola in w nella stringa S
    for w in words_s2:
        if w not in words_s1:
            return False
    return True
    '''
    counter_words=0
...
def norm_audio(audio):
    '''
    Function for audio normalization between -1 and 1
    :param audio: input audio [np.array]
    :return: output audio normalized [np.array]
    '''
...
def plotWave(wave, sr=16000, label="", xlabel="Time [sec]", ylabel="Amplitude", linestyle="-", marker=None):
    '''
    Function that plot the waveform of an audio
    :param wave: audio array
    :param sr: sample rate of audio, default = 16000
    :param label: title of the plot
    :param xlabel: time in seconds
    :param ylabel: amplitude
    :param linestyle: sttle of the line plot
    :param marker: None
    :return:
    '''
    # Compute the time in seconds
    num_samples = len(wave)
    time_seconds = np.arange(0, num_samples) / sr

    plt.figure()
    plt.plot(time_seconds, wave, linestyle=linestyle, marker=marker)
    plt.title(label)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.grid(False)
...
def kl_divergence(pred_sound: torch.Tensor, target_sound: torch.Tensor,
                  epsilon: float = 1e-6, reduction="mean") -> torch.Tensor:
    """Computes the elementwise KL-Divergence loss between two Mel spectrograms.

    Args:
        pred_sound (torch.Tensor): Spectrogram for generated audio.
        target_sound (torch.Tensor): Spectrogram for target audio.
        epsilon (float): Epsilon value to avoid numerical instability.
        reduction (string)(default="mean"): Kind of reduction ["none","sum","mean","batchmean"]

    Returns:
        kld (torch.Tensor): KL-divergence loss between each pair of generated and target Mel spectrograms.
    """
    pred_ready = pred_sound.log_softmax(0)
    target_ready = target_sound.softmax(0)
    return F.kl_div(pred_ready, target_ready, reduction=reduction)
...
#--------------------------------------------------------------------------------------------------------------------
def scegli_file_audio(percorso_cartella):
    # Verifica se il percorso fornito è una cartella
    ...
    files_audio = [f for f in os.listdir(percorso_cartella) if f.endswith('.mp3') or f.endswith('.wav')]
    # Verifica se ci sono file audio nella cartella
    if not files_audio:
        print("Non ci sono file audio nella cartella specificata.")
        return None
    # Scegli casualmente un file audio
    file_scelto = random.choice(files_audio)
    # Restituisci il percorso completo del file audio scelto
    percorso_file_scelto = os.path.join(percorso_cartella, file_scelto)
    return percorso_file_scelto

# Funzione di valutazione della fitness
def fitness_function(individual, target_tensor):
    diff = np.abs(individual - target_tensor)
    fitness = 1 / (np.sum(diff) + 1)
    return fitness


# Funzione di selezione dei genitori
def select_parents(population, target_tensor, num_parents):
    fitness_scores = [fitness_function(individual, target_tensor) for individual in population]
    selected_parents_indices = np.random.choice(len(population), size=num_parents, replace=False,
...
def mutate(individual, mutation_rate):
    ...
    individual += mutation_mask * np.random.randint(-1, 2, size=individual.shape)
    return individual
...
class AudioLDM2Pipeline(DiffusionPipeline,TextualInversionLoaderMixin):
        ...
        Args:
            ...
            generated_prompt_embeds (`torch.FloatTensor`, *optional*):
                 ...
                 *e.g.* prompt weighting. If not provided, text embeddings will be generated from `prompt` input
                 ...
                 The GPT-2 model produces output in the form of tensors with dimensions [batch_size, sequence_length, vocabulary_size].
                 Let's see what these dimensions correspond to:
                    batch_size:
                        Represents the number of text sequences in the batch.
                        During inference, this value can be 1 if you are generating a single text sequence.
                    sequence_length:
                        Indicates the maximum length of each text sequence.
                        For the GPT-2 model, which is a transformer-based attention model,
                        this dimension is fixed and corresponds to the maximum length of the sequence the model was trained on.
                        Longer sequences may be truncated or split into smaller batches.
                    vocabulary_size:
                        This dimension represents the size of the vocabulary,
                        i.e., the total number of different tokens the model can generate.
                        Each element of the tensors contains probability distributions over all possible tokens in the vocabulary.
    ...
    def mel_spectrogram_to_waveform(self, mel_spectrogram,prompt):
        ...
        # ----------------------------------------------------
        #board = Pedalboard([Reverb(room_size=0.45)])
        #print(board)
    ...
    def __call__(
        ...
        if not output_type == "latent":
            ...
            #print(f"Mel spectogram elements:",mel_spectrogram)
        else:
            return AudioPipelineOutput(audios=latents)

        audio , mel_spec = self.mel_spectrogram_to_waveform(mel_spectrogram,prompt)
        ...
        #10. Validation Test
        if (validation_path is not None) and (num_waveforms_per_prompt>1):
            print("Validation Test Start")
            out_path = (validation_path[:validation_path.rfind("/") + 1])
            #validation_path = (validation_path[:validation_path.rfind("/") + 1]) + "validation_set/"
            validation_path = (validation_path[:validation_path.rfind("/") + 1]) + "training_audio/"
            #---------------------------------------------------------------------------------------------
            #---------------------------------------------------------------------------------------------
